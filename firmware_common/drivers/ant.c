...
AntAssignChannelInfoType
    G_asAntChannelConfiguration[ANT_NUM_CHANNELS]; /*!< @brief Keeps track of
                                                      all configured ANT
                                                      channels */
AntMessageResponseType G_stAntMessageResponse;     /*!< @brief Holds the latest
                                                      message response info */

AntApplicationMsgListType
    *G_psAntApplicationMsgList; /*!< @brief Public linked list of messages from
                                   ANT to the application */
...
u8 G_au8AntSetNetworkKey[] = {
    MESG_NETWORK_KEY_SIZE, MESG_NETWORK_KEY_ID, 0 /* Network number */,
    0 /* KEY0 */,          0 /* KEY1 */,        0 /* KEY2 */,
    0 /* KEY3 */,          0 /* KEY4 */,        0 /* KEY5 */,
    0 /* KEY6 */,          0 /* KEY7 */,        CS};
u8 G_au8AntAssignChannel[] = {MESG_ASSIGN_CHANNEL_SIZE, MESG_ASSIGN_CHANNEL_ID,
                              0 /* AntChannel */,       0 /* AntChannelType */,
                              0 /* AntNetwork */,       CS};
u8 G_au8AntSetChannelID[] = {MESG_CHANNEL_ID_SIZE,        MESG_CHANNEL_ID_ID,
                             0 /* AntChannel */,          0 /* AntDeviceIdLo */,
                             0 /* AntDeviceIdHi */,       0 /* AntDeviceType */,
                             0 /* AntTransmissionType */, CS};
u8 G_au8AntSetChannelPeriod[] = {MESG_CHANNEL_MESG_PERIOD_SIZE,
                                 MESG_CHANNEL_MESG_PERIOD_ID,
                                 0 /* AntChannel */,
                                 0 /* AntChannelPeriodLo */,
                                 0 /* AntChannelPeriodHi */,
                                 CS};
u8 G_au8AntSetChannelRFFreq[] = {MESG_CHANNEL_RADIO_FREQ_SIZE,
                                 MESG_CHANNEL_RADIO_FREQ_ID, 0 /* AntChannel */,
                                 0 /* AntFrequency */, CS};
u8 G_au8AntSetChannelPower[] = {MESG_RADIO_TX_POWER_SIZE,
                                MESG_RADIO_TX_POWER_ID, 0 /* AntChannel */,
                                0 /* AntTxPower */, CS};
u8 G_au8AntSetSearchTimeout[] = {
    MESG_CHANNEL_SEARCH_TIMEOUT_SIZE, MESG_CHANNEL_SEARCH_TIMEOUT_ID,
    0 /* AntChannel */, 0xFF /* SearchTime (0xFF = infinite) */, CS};
u8 G_au8AntLibConfig[] = {MESG_LIB_CONFIG_SIZE, MESG_LIB_CONFIG_ID, 0,
                          LIB_CONFIG_CHANNEL_ID_FLAG | LIB_CONFIG_RSSI_FLAG,
                          CS};

u8 G_au8AntBroadcastDataMessage[] = {MESG_DATA_SIZE,
                                     ...
                                     D_7,
                                     CS};
u8 G_au8AntAckDataMessage[] = {MESG_DATA_SIZE,
                               ...
                               D_7,
                               CS};
u8 G_au8ANTGetVersion[] = {MESG_REQUEST_SIZE, MESG_REQUEST_ID, 0,
                           MESG_VERSION_ID, 0};
/*! @endcond */

/*----------------------------------------------------------------------------*/
/* Existing variables (defined in other files -- should all contain the "extern"
keyword) and indicate what file the variable is defined in. */
...
***********************************************************************************************************************/
static fnCode_type
    Ant_pfnStateMachine;   /*!< @brief The ANT state machine function pointer */
...
static u32 Ant_u32TxByteCounter =
    0; /*!< @brief Counter counts callbacks on sent bytes */
static u32 Ant_u32RxByteCounter =
    0; /*!< @brief Counter counts callbacks on received bytes */
static u32 Ant_u32RxTimeoutCounter =
    0; /*!< @brief Increments any time an ANT reception times out */
static u32 Ant_u32UnexpectedByteCounter =
    0; /*!< @brief Increments any time a byte is received that was not expected
          value */
static u32 Ant_u32CurrentTxMessageToken =
    0; /*!< @brief Token for message currently being sent to ANT */

static SspConfigurationType
    Ant_sSspConfig; /*!< @brief Configuration information for SSP peripheral */
static SspPeripheralType
    *Ant_Ssp; /*!< @brief Pointer to Ant's SSP peripheral object */

static u8 Ant_u8AntVersion[MESG_VERSION_SIZE]; /*!< @brief ANT device version
                                                  string */

static u8
    Ant_au8AntRxBuffer[ANT_RX_BUFFER_SIZE]; /*!< @brief Space for received ANT
                                               message bytes used by SSP */
static u8
    *Ant_pu8AntRxBufferNextChar; /*!< @brief Pointer to next char to be written
                                    in the AntRxBuffer used by SSP */

static u8 *Ant_pu8AntRxBufferCurrentChar; /*!< @brief Pointer to the current
                                             char in the AntRxBuffer */
static u8 *Ant_pu8AntRxBufferUnreadMsg;   /*!< @brief Pointer to unread chars in
                                             the AntRxBuffer */
static u8 Ant_u8AntNewRxMessages; /*!< @brief Counter for number of new messages
                                     in AntRxBuffer */

static u32 Ant_u32ApplicationMessageCount =
    0; /*!< @brief Counts messages queued on G_psAntApplicationMsgList */
static AntOutgoingMessageListType
    *Ant_psOutgoingMsgList; /*!< @brief Linked list of outgoing ANT-formatted
                               messages */
static u32 Ant_u32OutgoingMessageCount =
    0; /*!< @brief Counts messages queued on Ant_psOutgoingMsgList */

static u8 Ant_u8SlaveMissedMessageHigh =
    0; /*!< @brief Counter for missed messages if device is a slave */
static u8 Ant_u8SlaveMissedMessageMid =
    0; /*!< @brief Counter for missed messages if device is a slave */
static u8 Ant_u8SlaveMissedMessageLow =
    0; /*!< @brief Counter for missed messages if device is a slave */
...
/* Debug variables */
static u32 Ant_DebugRxMessageCounter =
    0; /*!< @brief Number of visits to the RxMessage state */
static u32 Ant_DebugTotalRxMessages =
    0; /*!< @brief Number of message received in AntRxMessage */
static u32 Ant_DebugProcessRxMessages =
    0; /*!< @brief Calls to AntProcessMessage */
static u32 Ant_DebugQueuedDataMessages =
    0; /*!< @brief Calls to AntQueueOutgoingMessage */
...
@brief Creates a new ANT message structure and adds it into
...
@param pu8Message_ is an ANT-formatted message starting with LENGTH and ending
with CHECKSUM

Promises:
- A new list item in the outgoing message linked list is created and inserted at
the end of the list as long as there is enough room.
...
bool AntQueueOutgoingMessage(u8 *pu8Message_) {
  ...
  /* Otherwise traverse the list to find the end where the new message will be
     inserted */
  else {
...
Requires:
- G_psAntApplicationMsgList points to the start of the list which is the entry
to remove
...
void AntInitialize(void) {
  ...
  /* Otherwise try to start up ANT normally */
    ...
    /* Announce on the debug port that ANT setup is starting and intialize
     * pointers */
    ...
    for (u8 i = 0; i < ANT_NUM_CHANNELS; i++) {
      ...
      G_asAntChannelConfiguration[i].AntTransmissionType =
          ANT_TRANSMISSION_TYPE_DEFAULT;
      G_asAntChannelConfiguration[i].AntChannelPeriodLo =
          ANT_CHANNEL_PERIOD_LO_DEFAULT;
      G_asAntChannelConfiguration[i].AntChannelPeriodHi =
          ANT_CHANNEL_PERIOD_HI_DEFAULT;
...
The peripheral task sending the message must invoke this function after each
byte.

Note: Since this function is called from an ISR, it should execute as quickly as
possible.
...
The peripheral task receiving the message must invoke this function after each
byte.

Note: Since this function is called from an ISR, it should execute as quickly as
possible. Unfortunately, AntSrdyPulse() takes some time but the duty cycle of
this interrupt is low enough that we can survive (this interrupt priority could
be dropped below everything else to mitigate any issues).

Requires:
- G_u32AntFlags _ANT_FLAGS_RX_IN_PROGRESS is correctly set
- A received byte was just written to the Rx buffer
- _SSP_CS_ASSERTED already in correct state: should be set on first byte, but
application should clear it for all subsequent bytes so flow control is handled
entirely by this function
- ISRs are off already since this is totally not re-entrant

Promises:
- Ant_pu8AntRxBufferNextChar is advanced safely so it is ready to receive the
next byte
...
*/
void AntRxFlowControlCallback(void) {
  /* Count the byte and safely advance the receive buffer pointer; this is
  called from the RX ISR, so it won't be interrupted and break
...
Due to the speed of the chip-to-chip communications, even the longest ANT
message should be able to send in less than 500us so it will likely be done on
the main program cycle that immediately follows this call.

Requires:
@param pu8AntTxMessage_ points to an Ant formatted message where the first data
byte is the length byte (since ANT sends the SYNC byte) and the last byte is the
checksum.

Promises:
- MRDY is de-asserted
- Returns TRUE if the transmit message is queued successfully;
Ant_u32CurrentTxMessageToken holds the message token
- Returns FALSE if the transfer couldn't start or if receive message interrupted
  (G_u32AntFlags _ANT_FLAGS_TX_INTERRUPTED is set;  AntRxBufferCurrentChar
pointing to the received byte).
...
static bool AntTxMessage(u8 *pu8AntTxMessage_) {
  ...
  /* Check G_u32AntFlags first to ensure a transmission is not already taking
   * place */
  ...
  /* Initialize the timeout timer and notify ANT that the Host wishes to send a
   * message */
  Ant_u32RxTimer = 0;
  SYNC_MRDY_ASSERT();
  ...
  /* Else we have SEN flag; queue to read 1 byte after a short delay before
   * toggling SRDY */
  AntSrdyPulse();
  ...
  /* If we timed out now, then clear MRDY and exit.  Because CS is still
  asserted, the task will attempt to read a message but fail and eventually
  abort. */
  if (Ant_u32RxTimer > ANT_ACTIVITY_TIME_COUNT) {
    DebugPrintf(au8TxTimeoutMsg);
    return (FALSE);
  ...
  /* When the byte comes in, the SSP module will set the _SSP_RX_COMPLETE flag
  and also call the Rx callback but does not toggle SRDY at this time.  We must
  look at this byte to determine if ANT initiated this particular communication
  and is telling us that a message is coming in, or if we initiated the
  communication and ANT is allowing us to transmit. */

  /* Read the byte - don't advance the pointer yet */
  u8Byte = *Ant_pu8AntRxBufferCurrentChar;

  /* If the byte is TX_SYNC, then ANT wants to send a message which must be done
   * first */
  ...
  /* Since the Rx byte is in our Rx buffer, advance both pointers since it's not
   * an incoming message */
  ...
  if (u8Byte == MESG_RX_SYNC) {
    ...
    /* Read the message length and add three for the length, message ID and
     * checksum */
    u32Length = (u32)(pu8AntTxMessage_[0] + 3);

    /* Queue the message to the peripheral and capture the token */
    Ant_u32CurrentTxMessageToken =
        SspWriteData(Ant_Ssp, u32Length, pu8AntTxMessage_);

    /* Return TRUE only if we received a message token indicating the message
     * has been queued */
...
- G_u32AntFlags _ANT_FLAGS_TX_INTERRUPTED is set if the system wanted to
transmit but ANT wanted to send a message at the same time (so MESG_TX_SYNC has
already been received); _SSP_RX_COMPLETE must still be set from this.
- Ant_pu8AntRxBufferCurrentChar points to the first byte of the message

Promises:
- If a good new message has been received, then Ant_u8AntNewMessages is
incremented and the message is at Ant_pu8AntRxBufferUnreadMsg in
Ant_au8AntRxBuffer
- If a good message is not received, then Ant_u8AntNewMessages is unchanged.
- In both cases, Ant_pu8AntRxBufferNextChar points at the next empty buffer
location
...
static void AntRxMessage(void) {
  ...
  /* If the Global _ANT_FLAGS_TX_INTERRUPTED flag has been set, then we have
   * already read the TX_SYNC byte */
  if (G_u32AntFlags & _ANT_FLAGS_TX_INTERRUPTED) {
    /* Clear flag and adjust the starting byte counter since the message came in
     * during AntTxMessage*/
    G_u32AntFlags &= ~_ANT_FLAGS_TX_INTERRUPTED;
  ...
  /* Otherwise we need to first read the sync byte  */
    ...
    /* Begin the receive cycle that takes place using interrupts and callbacks
    and is monitored by a timeout of about 500us - this should be plenty of time
    to receive even the longest ANT message but still only half the allowed 1ms
    loop time for the system. */
  ...
  Ant_pu8AntRxBufferCurrentChar. Proceed to test it and receive the rest of the
  message */
  if (*Ant_pu8AntRxBufferCurrentChar == MESG_TX_SYNC) {
    ...
    /* We block here while the SSP interrupts and Rx callback handle the rest of
    the reception until a full message is received. We know it is received when
    SEN is deasserted. This takes about 500us */
    while (IS_SEN_ASSERTED() && (Ant_u32RxTimer < ANT_ACTIVITY_TIME_COUNT)) {
      Ant_u32RxTimer++;
    }
    ...
    if (Ant_u32RxTimer < ANT_ACTIVITY_TIME_COUNT) {
      ...
      /* RxBufferCurrentChar is still pointing to the SYNC byte. Validate what
       * should be a complete message now. */
      u8Checksum = *Ant_pu8AntRxBufferCurrentChar;
      AdvanceAntRxBufferCurrentChar();

      /* Read the length byte and add two to count the length byte and message
       * ID but not checksum as length will be our checksum counter */
      u8Length = *Ant_pu8AntRxBufferCurrentChar + 2;

      /* Optional check (u8Length does not include the SYNC byte or Checksum
       * byte so add 2) */
      if (u32CurrentRxByteCount != (u8Length + 2)) {
        /* Could throw out the message right away - this could save some
        potential weird memory accesses if there was any corruption or a wild
        u8Length value */
      ...
      /* AntRxBufferCurrentChar is pointing to the last received byte that
       * should be the checksum. */
      if (u8Checksum == *Ant_pu8AntRxBufferCurrentChar) {
        Ant_u8AntNewRxMessages++;
        Ant_DebugTotalRxMessages++;
      }
      /* If the message was not good, then move Ant_pu8AntRxBufferUnreadMsg
         passed the garbage data */
      else {
    ...
    /* Otherwise we have received an unexpected byte -- flag it, clear Ssp flags
     * and abandon the byte */
...
@brief Kills the current message in progress with ANT and resets all of the
pointers.
...
Promises:
- Ant_pu8AntRxBufferNextChar, Ant_pu8AntRxBufferCurrentChar,
Ant_pu8AntRxBufferUnreadMsg, and Ant_u8AntNewRxMessages reset.
...
Promises:
- Ant_pu8AntRxBufferCurrentChar points to the next char to write in
Ant_au8AntRxBuffer

*/
static void AdvanceAntRxBufferCurrentChar(void) {
  Ant_pu8AntRxBufferCurrentChar++;
  if (Ant_pu8AntRxBufferCurrentChar ==
      &Ant_au8AntRxBuffer[ANT_RX_BUFFER_SIZE]) {
    Ant_pu8AntRxBufferCurrentChar = &Ant_au8AntRxBuffer[0];
  }
...
@brief Waits a specified amount of time for a particular message to arrive from
ANT in response to a message sent to ANT. ***This function violates the 1ms
system rule, so should only be used during initialization.***
...
- SSP task should be in manual mode so it is busy sending the Tx message to
which this function will wait for the ANT response.

@param u8ExpectedMessageID_ is the ID of a message to which a response is
expected
...
static u8 AntExpectResponse(u8 u8ExpectedMessageID_, u32 u32TimeoutMS_) {
  ...
  if (!bTimeout) {
    ...
    /* If there is a new message in the receive buffer, then check that it is a
    response to the expected message and that the response is no error */
...
#endif
      /* 2017-11-18 Fix to handle Events and responses but also just message
      replys so ExpectResponse will still return correctly for regular messages
    */

      /* Check if the response is an Event */
      if (*(Ant_pu8AntRxBufferUnreadMsg + MESG_ID_OFFSET) ==
          MESG_RESPONSE_EVENT_ID) {
        /* For Event messages, check if it is a reply to the expected message
         * and the reply is good. */
        if ((*(Ant_pu8AntRxBufferUnreadMsg + MESG_RESPONSE_MESG_ID_OFFSET) ==
             u8ExpectedMessageID_) &&
            (*(Ant_pu8AntRxBufferUnreadMsg + MESG_RESPONSE_CODE_OFFSET) ==
             RESPONSE_NO_ERROR)) {
          u8ReturnValue = 0;
        }
      }
      /* If not an event, the message should be a response to the requested
         message */
      else if (*(Ant_pu8AntRxBufferUnreadMsg + MESG_ID_OFFSET) ==
               u8ExpectedMessageID_) {
        u8ReturnValue = 0;
      }
...
@brief Reads the latest received Ant message and updates system information
accordingly.

Requires:
- Ant_u8AntNewRxMessages holds the number of unprocessed messages in the message
queue
- Ant_pu8AntRxBufferUnreadMsg points to the first byte of an unread verified ANT
message

Promises:
- Returns 1 if Ant_u8AntNewRxMessages == 0 or the message exceeds the maximum
allowed length
- Returns 0 otherwise and:
  - Ant_u8AntNewRxMessages--
  - Ant_pu8AntRxBufferUnreadMsg points to the first byte of the next unread
verified ANT message
...
static u8 AntProcessMessage(void) {
  ...
  for (u8 i = 0; i < (u8MessageLength + MESG_FRAME_SIZE - MESG_SYNC_SIZE);
       i++) {
    au8MessageCopy[i] = *Ant_pu8AntRxBufferUnreadMsg;
    AdvanceAntRxBufferUnreadMsgPointer();
  }
  /* Note: Ant_pu8AntRxBufferUnreadMsg is now pointing at the next unread
   * message */
  ...
  switch (au8MessageCopy[BUFFER_INDEX_MESG_ID]) {
    case MESG_RESPONSE_EVENT_ID: {
      ...
      if (au8MessageCopy[BUFFER_INDEX_RESPONSE_MESG_ID] != MESG_EVENT_ID) {
        /* We have a Channel Response: parse it out based on the message ID to
        which the response applies and post the result */
        G_stAntMessageResponse.u8Channel = u8Channel;
        G_stAntMessageResponse.u8MessageNumber =
            au8MessageCopy[BUFFER_INDEX_RESPONSE_MESG_ID];
        ...
        switch (au8MessageCopy[BUFFER_INDEX_RESPONSE_MESG_ID]) {
          ...
          case MESG_OPEN_CHANNEL_ID: {
            ...
            if (au8MessageCopy[BUFFER_INDEX_RESPONSE_CODE] ==
                RESPONSE_NO_ERROR) {
          ...
          case MESG_CLOSE_CHANNEL_ID: {
            G_au8AntMessageClose[12] =
                au8MessageCopy[BUFFER_INDEX_CHANNEL_NUM] + 0x30;
          ...
          default: {
            ...
            G_au8AntMessageUnhandled[24] = HexToASCIICharLower(
                (au8MessageCopy[BUFFER_INDEX_RESPONSE_MESG_ID] >> 4) & 0x0F);
            G_au8AntMessageUnhandled[25] = HexToASCIICharLower(
                (au8MessageCopy[BUFFER_INDEX_RESPONSE_MESG_ID] & 0x0F));
            G_au8AntMessageUnhandled[36] = HexToASCIICharLower(
                (au8MessageCopy[BUFFER_INDEX_RESPONSE_CODE] >> 4) & 0x0F);
            G_au8AntMessageUnhandled[37] = HexToASCIICharLower(
                (au8MessageCopy[BUFFER_INDEX_RESPONSE_CODE] & 0x0F));
      ...
      /* The message is a Channel Event, so the Event Code must be parsed out */
        ...
        switch (au8MessageCopy[BUFFER_INDEX_RESPONSE_CODE]) {
          ...
          case EVENT_RX_FAIL: /* Slave did not receive a message when expected
                               */
          {
            /* The Slave missed a message it was expecting: communicate this to
            the application in case it matters. Could also queue a debug message
            here. */
          ...
          case EVENT_RX_FAIL_GO_TO_SEARCH: /* Slave has lost sync with Master
                                              (channel still open) */
          {
            /* The Slave missed enough consecutive messages so it goes back to
            search: communicate this to the application in case it matters.
            Could also queue a debug message here. */
            AntTickExtended(au8MessageCopy);
#ifdef ANT_VERBOSE
          ...
          {
            /* If this is a Master device, then EVENT_TX means it's time to
          ...
          case EVENT_TRANSFER_TX_COMPLETED: /* ACK received from an acknowledged
                                               data message */
          {
            G_asAntChannelConfiguration[u8Channel].AntFlags |=
                _ANT_FLAGS_GOT_ACK;
          ...
          case EVENT_TRANSFER_TX_FAILED: /* ACK was not received from an
                                            acknowledged data message */
          {
            /* Regardless of complete or fail, it is time to send the next
             * message */
            AntTickExtended(au8MessageCopy);
#ifdef ANT_VERBOSE
          ...
          case EVENT_RX_SEARCH_TIMEOUT: /* The ANT channel is going to close due
                                           to search timeout */
          {
    ...
    case MESG_BROADCAST_DATA_ID: /* A broadcast data message was received */
    {
      ...
      AntQueueExtendedApplicationMessage(
          ANT_DATA, &au8MessageCopy[BUFFER_INDEX_MESG_DATA], &sExtendedData);
...
this for future development. */
    ...
    case MESG_CHANNEL_STATUS_ID: /* Message sent in response to a channel status
                                    request */
    {
      break;
...
/*!------------------------------------------------------------------------------
@fn static bool AntParseExtendedData(u8* pu8SourceMessage_, AntExtendedDataType*
psExtDataTarget_)
...
extended information, so this code is commented out but in the correct location
if needed.

Requires:
@param pu8SourceMessage_ points to an ANT message buffer that holds a complete
ANT data message structure except for SYNC byte; therefore buffer indices from
antmessage.h can be used.
...
- Returns FALSE if no extended data is present; psExtDataTarget_ is set to
default values

*/
static bool AntParseExtendedData(u8 *pu8SourceMessage_,
                                 AntExtendedDataType *psExtDataTarget_) {
  bool bReturnValue;
  u8 u8MessageSize;
  u8 u8Channel;
  u8 u8Flags = 0;
  ...
  /* Otherwise we have some extended message data */
    ...
    if (u8Flags & LIB_CONFIG_CHANNEL_ID_FLAG) {
      u16DeviceID =
          (u16)(*(pu8SourceMessage_ + BUFFER_INDEX_EXT_DATA + u8BufferOffset)) &
          0x00FF;
      u8BufferOffset++;
      u16DeviceID |=
          ((u16)(*(pu8SourceMessage_ + BUFFER_INDEX_EXT_DATA + u8BufferOffset))
      ...
      u8BufferOffset++;
      u8DeviceType =
          *(pu8SourceMessage_ + BUFFER_INDEX_EXT_DATA + u8BufferOffset);
      u8BufferOffset++;
      u8TransType =
          *(pu8SourceMessage_ + BUFFER_INDEX_EXT_DATA + u8BufferOffset);
      u8BufferOffset++;
    ...
    if (u8Flags & LIB_CONFIG_RSSI_FLAG) {
      // u8MeasurementType = *(pu8SourceMessage_ + BUFFER_INDEX_EXT_DATA +
      // u8BufferOffset);
      u8BufferOffset++;
      s8RSSI = *(pu8SourceMessage_ + BUFFER_INDEX_EXT_DATA + u8BufferOffset);
      u8BufferOffset++;
      // u8Threshold = *(pu8SourceMessage_ + BUFFER_INDEX_EXT_DATA +
      // u8BufferOffset);
      u8BufferOffset++;
    }
...
/*!-----------------------------------------------------------------------------/
@fn static bool AntQueueExtendedApplicationMessage(AntApplicationMessageType
eMessageType_, u8* pu8DataSource_, AntExtendedDataType* psExtData_)

@brief Creates a new ANT message structure and adds it to
G_psAntApplicationMsgList.

The Application list is used to communicate message information between the ANT
driver and the ANT_API simplified interface task.  It has room for
ANT_APPLICATION_MESSAGE_BUFFER_SIZE messages. The messages are either ANT_DATA
or ANT_TICK messsage and include all information and data from the original ANT
message.
...
@param pu8DataSource_ is a pointer to the first element of an array of 8 data
bytes
...
*/
static bool AntQueueExtendedApplicationMessage(
    AntApplicationMessageType eMessageType_, u8 *pu8DataSource_,
...
static void AntTickExtended(u8 *pu8AntMessage_) {
  ...
  au8Message[ANT_TICK_MSG_CHANNEL_INDEX] =
      *(pu8AntMessage_ + BUFFER_INDEX_CHANNEL_NUM);
  au8Message[ANT_TICK_MSG_RESPONSE_TYPE_INDEX] =
      *(pu8AntMessage_ + BUFFER_INDEX_RESPONSE_MESG_ID);
  au8Message[ANT_TICK_MSG_EVENT_CODE_INDEX] =
      *(pu8AntMessage_ + BUFFER_INDEX_RESPONSE_CODE);
  au8Message[ANT_TICK_MSG_SENTINEL_INDEX] = MESSAGE_ANT_TICK_SENTINEL;
  au8Message[ANT_TICK_MSG_MISSED_HIGH_BYTE_INDEX] =
      Ant_u8SlaveMissedMessageHigh;
  ...
  AntQueueExtendedApplicationMessage(
      ANT_TICK, &au8Message[ANT_TICK_MSG_ID_INDEX], &sExtData);
...
@brief Properly sets up the ANT SPI interface and tests Host <-> ANT
communications.
...
- If ANT starts up correctly and responds to version request, then
  G_u32SystemFlags _APPLICATION_FLAGS_ANT is set and Ant_u8AntVersion is
populated with the returned version information from the ANT IC.
...
@brief Idle state that will process new messages if any are present, monitors
ANT for incoming messages, and sends broadcast messages that are waiting to be
sent. Incoming messages from ANT always get priority.
*/
...
@brief Completely receive an ANT message. Reception is very fast and should
complete in less than 600us for a 15-byte message.  AntRxMessage could just be
called from Idle, but giving it its own state minimizes the total time and
allows for easier future updates should they be required.
*/
...
@brief Wait for an ANT message to be transmitted.  This state only occurs once
the handshaking transaction has been completed and transmit to ANT is verified
and underway.
*/
static void AntSM_TransmitMessage(void) {
  ...
  switch (eCurrentMsgStatus) {
    ...
    case COMPLETE: {
      ...
      transaction.  This takes about 170us, so block in this state until that's
      over */
...
@brief Do-nothing state if ANT is dead (requires restart to retry
initialization)
*/
static void AntSM_NoResponse(void) {} /* end AntSM_NoResponse() */
